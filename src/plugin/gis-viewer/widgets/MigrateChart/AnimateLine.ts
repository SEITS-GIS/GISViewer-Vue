...
export default class AnimateLine {
  ...
  public static getInstance(view: any) {
    ...
    if (!AnimateLine.intances) {
      AnimateLine.intances = new Map();
    }
    let intance = AnimateLine.intances.get(id);
    if (!intance) {
      intance = new AnimateLine(view);
      AnimateLine.intances.set(id, intance);
    }
    return intance;
  }
  private clear() {}
  private async addAnimateLine(params: any) {
    const [webMercatorUtils, Graphic, geometryJsonUtils] = await loadModules([
      'esri/geometry/support/webMercatorUtils',
      'esri//Graphic',
      'esri/geometry/support/jsonUtils'
    ]);
    let overlays = params.overlays;
    let graphics = overlays.map((overlay: any) => {
      overlay.geometry.spatialReference = {wkid: '4326'};
      overlay.geometry.type = 'polyline';
      let geometry = geometryJsonUtils.fromJSON(overlay.geometry);
      return {
        attributes: {
          color: overlay.symbol.color
        },
        symbol: {type: 'simple-line', color: 'white', width: 3},
        geometry: webMercatorUtils.geographicToWebMercator(geometry)
      };
    });
    this.adds(graphics);
  }
  private async adds(graphics: any) {
    let _this = this;
    const [
      BaseLayerViewGL2D,
      watchUtils,
    ...
    let CustomLayerView2D = BaseLayerViewGL2D.createSubclass({
      ...
      constructor: function() {
        ...
        this.extrude = glMatrix.mat3.create();
      ...
      render: function(renderParameters: any) {
        ...
        glMatrix.mat3.identity(this.extrude);
        glMatrix.mat3.rotate(
          this.extrude,
          this.extrude,
          (Math.PI * state.rotation) / 180
        ...
        const HALF_WIDTH = 6;
        glMatrix.mat3.scale(this.extrude, this.extrude, [
          HALF_WIDTH,
          -HALF_WIDTH,
          1
        ]);
    ...
    let clayer = new CustomLayer({
      graphics: graphics
    });

    let clayer2 = new GraphicsLayer({
      graphics: graphics
    });
    _this.view.map.layers.add(clayer2);
    _this.view.map.layers.add(clayer);
  }
}
